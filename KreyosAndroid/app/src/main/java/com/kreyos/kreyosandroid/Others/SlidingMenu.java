...
package com.kreyos.kreyosandroid.others;
...
public class SlidingMenu extends LinearLayout {
	...
	private static class SlideMenuAdapter extends ArrayAdapter<SlideMenuItem> {
		...
		public static boolean mIsLeft = true;
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			&& position == 0) {
				if (AppHelper.instance().WATCH_STATE == WATCH_STATE_VALUE.CONNECTED) {
					holder.label.setText("WATCH CONNECTED");
				} else {
					holder.label.setText("WATCH DISCONNECTED");
				}
			}
			*/
	...
	private TranslateAnimation slideLeftAnim;
	private TranslateAnimation slideMenuRightAnim;
	private TranslateAnimation slideContentRightAnim;


	private ArrayList<SlideMenuItem> menuItemList;
	private SlideMenuInterface.OnSlideMenuItemClickListener callback;
	
	private boolean IsSlidingInLeft = true;
	 ...
	 */
	public SlidingMenu(Activity act, int menuResource, SlideMenuInterface.OnSlideMenuItemClickListener cb, int slideDuration) {
		super(act);
	 ...
	 * @param slideDuration Slide in/out duration in milliseconds.
	 */
	public SlidingMenu(Activity act, SlideMenuInterface.OnSlideMenuItemClickListener cb, int slideDuration) {
	...
	public void init(Activity act, int menuResource, SlideMenuInterface.OnSlideMenuItemClickListener cb, int slideDuration) {
		...
		slideLeftAnim = new TranslateAnimation(menuSize, 0, 0, 0);
		slideLeftAnim.setFillAfter(true);
		slideMenuRightAnim = new TranslateAnimation(0, menuSize, 0, 0);
		slideMenuRightAnim.setFillAfter(true);
		slideContentRightAnim = new TranslateAnimation(-menuSize, 0, 0, 0);
		slideContentRightAnim.setFillAfter(true);
	...
	public void init(Activity act, int menuResource, SlideMenuInterface.OnSlideMenuItemClickListener cb, int slideDuration, boolean isLeft ) {

		this.act = act;
		this.callback = cb;
		IsSlidingInLeft = isLeft;
	...
	public void setAnimationDuration(long slideDuration) {
		if( IsSlidingInLeft ) {
			...
			slideLeftAnim.setDuration(slideDuration);
			slideMenuRightAnim.setDuration(slideDuration*3/2);
			slideContentRightAnim.setDuration(slideDuration*3/2);
		}
	}
	...
	public void setAnimationInterpolator(Interpolator i) {
		if( IsSlidingInLeft ){
			...
			slideLeftAnim.setInterpolator(i);
			slideMenuRightAnim.setInterpolator(i);
			slideContentRightAnim.setInterpolator(i);
		}
	}
	...
	private void show(boolean animate) {
		...
		if (IsSlidingInLeft)  {
			...
			// short targetMargin = (short)(content.getWidth() * 0.3f);
			// Fixed slide margin on right side
			lays.setMargins(content.getWidth() - menuSize, statusHeight, 0, 0);
			// Add click listener
		...
		SlideMenuAdapter.mIsLeft = IsSlidingInLeft;
		...
		if(animate) {
			if(IsSlidingInLeft) {
				...
				menu.startAnimation(slideLeftAnim);
			}
		}
	...
	public void hide() {
		...
		if( IsSlidingInLeft ) {
			...
			menu.startAnimation(slideMenuRightAnim);
		}
		...
		if( IsSlidingInLeft ) {
			...
			content.startAnimation(slideContentRightAnim);
		}
	...
	private void show1(boolean animate) {
		...
		if( IsSlidingInLeft ) {
			...
			lays.setMargins(100, statusHeight, 0, 0);
		}
		menu.setLayoutParams(lays);
